...
| File | Descrizione |
|---|---|
| `cache_semantica.py` | La classe `CacheSemantica`, che indicizza le risposte per similarita' semantica della domanda. Sezione "Latenza". |
| `routing_per_complessita.py` | Le costanti dei modelli, il `PROMPT_ROUTER` e la funzione `instrada` che smista le richieste tra modello piccolo e grande. Sezione "Costi". |
| `catena_di_fallback.py` | La `CATENA_FALLBACK` e la funzione `genera_con_fallback` che percorre i gradini fino al provider indipendente. Sezione "Affidabilita'". |
